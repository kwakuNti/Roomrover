...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (count($likes) < 2 || count($dislikes) < 2 || count($knows) < 2) {
        header('Location: ../templates/preferencesx.php?msg=Please select at least two preferences from each category.');
        exit;
    }
    ...
    header('Location: ../templates/profile.php?msg=Preferences saved successfully.');
    exit;
}
?>
