...
if ($totalUsers != 1 && $totalUsers != 2 && $totalUsers != 4) {
    header("Location: ../templates/oteng_korankye.php?msg=Invalid number of users for booking.");
    exit();
}
...
try {
    ...
    header("Location: ../templates/oteng_korankye.php?msg=You have successfully booked the room.");
    exit();
} catch (Exception $e) {
    // Rollback transaction if something goes wrong
    $conn->rollback();
    header("Location: ../templates/oteng_korankye.php?msg=" . urlencode($e->getMessage()));
    exit();
}
