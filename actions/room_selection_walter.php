...
if ($totalUsers != 1 && $totalUsers != 2 && $totalUsers != 4) {
    header("Location: ../templates/walter_sisulu.php?msg=Invalid number of users for booking.");
    exit();
}
...
try {
    ...
    header("Location: ../templates/walter_sisulu.php?msg=You have successfully booked the room.");
    exit();
} catch (Exception $e) {
    // Rollback transaction if something goes wrong
    $conn->rollback();
    header("Location: ../templates/walter_sisulu.php?msg=" . urlencode($e->getMessage()));
    exit();
}
