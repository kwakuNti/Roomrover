...
if ($totalUsers != 1 && $totalUsers != 2 && $totalUsers != 4) {
    header("Location: ../templates/wangari_mathai.php?msg=Invalid number of users for booking.");
    exit();
}
...
try {
    ...
    $conn->rollback();
    header("Location: ../templates/wangari_mathai.php?msg=" . urlencode($e->getMessage()));
    exit();
}
